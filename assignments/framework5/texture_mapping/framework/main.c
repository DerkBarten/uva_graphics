...
InitGL(void)
{
    ...
    for (i = 0; texture_files[i].filename != NULL; i++)
    {
        ...
        if (image_data)
        {
            ...
            // GL_REPEAT repeats the textures instead of stretching
...
DrawGLScene(void)
{
    ...
    for (int t = 0; t < 12; t++)
    {
        ...
        // How many leaves to draw
        int leaves = 8;
        // How much rotation per leaf
        int rotation = 360.0 / leaves;
        for (int i = 0; i < leaves; i ++) {
            glRotatef(rotation * i, 0, 1, 0);
            // Draw the specified number of leaves
